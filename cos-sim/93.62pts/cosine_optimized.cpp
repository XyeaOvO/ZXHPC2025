...
float dot_product_avx512(const float *a, const float *b, int D) {
...
int main() {
    ...
    std::cin.read(reinterpret_cast<char *>(data.data()), (long)N * D * sizeof(float));

    std::vector<float> inv_norms(N);
    #pragma omp parallel for schedule(static)
    for (uint32_t i = 0; i < N; ++i) {
        float norm_sq = dot_product_avx512(data.data() + (long)i * D, data.data() + (long)i * D, D);
            ...
            if (i_block == j_block) {
                for (int i = i_block; i < i_max; ++i) {
                    for (int j = i; j < j_max; ++j) {
                        float dp = dot_product_avx512(data.data() + (long)i * D, data.data() + (long)j * D, D);
