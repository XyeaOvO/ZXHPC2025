...
float dot_product_avx_aligned(const float *a, const float *b, int D) {
...
int main() {
    ...
    for (uint32_t i = 0; i < N; ++i) {
        float norm_sq = dot_product_avx_aligned(data + (long)i * D, data + (long)i * D, D);
            ...
            if (i_block == j_block) {
                for (int i = i_block; i < i_max; ++i) {
                    for (int j = i; j < j_max; ++j) {
                        float dp = dot_product_avx_aligned(data + (long)i * D, data + (long)j * D, D);
                ...
                for (int i = i_block; i < i_max; ++i) {
                    ...
                    for (; j <= j_max - 2; j += 2) {
                        ...
                        float dp0 = dot_product_avx_aligned(vec_i, vec_j0, D);
                        dot_products[(long)i * N + j] = dp0;
                        dot_products[(long)j * N + i] = dp0;

                        const float* vec_j1 = data + (long)(j + 1) * D;
                        float dp1 = dot_product_avx_aligned(vec_i, vec_j1, D);
